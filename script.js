...
//     filteredTasks = tasks.filter(task => task.text.toLowerCase().includes(searchText.toLowerCase()))
//     renderTasks();
// }
...
function renderTasks() {
    ...
    // Отображаем найденные задачи
    const searchText = searchInput.value.trim();
    if (searchText != ''){
        filteredTasks = filteredTasks.filter(task => 
            String(task.text).toLowerCase().includes(searchText.toLowerCase())
            || String(task.assignee).toLowerCase().includes(searchText.toLowerCase()));
    }
